...
export async function streamExport(opts: ExportOptions): Promise<ExportResult> {
  const { db, chatIdentifier, format, outputPath, since, until, pageSize } = opts;
  ...
  let beforeMessageId: number | undefined = undefined;
  ...
  if (format === "markdown") {
    ...
    await writeAndDrain(stream, "id,date,sender,handle,is_from_me,is_read,is_reply,reply_to_text,text,has_attachments\n");
  } else if (format === "json") {
  ...
  while (true) {
    ...
    if (filtered.length > 0) {
      if (format === "markdown") {
        await writeAndDrain(stream, toMarkdown(filtered, { thread: chatIdentifier }).split("\n").slice(7).join("\n"));
