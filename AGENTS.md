...
**What:** Each chat gets a slug like `mona~imsg~a3f2` or `weekend-crew~imsg~d4e5` (see `src/thread-slug.ts`: sanitized name + service abbrev + short hash of GUID). `list_conversations` includes **`threadSlug`** for each row.
