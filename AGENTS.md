...
- **Media**: `src/media.ts` – zero-dep macOS helpers (sips/qlmanage/mdls) turning attachments into MCP image blocks, video poster frames, and optional audio transcripts. On-device transcribers (hear/yap/whisper-cli) are auto-detected and always preferred; when none is installed AND `IMSG_TRANSCRIBE_PROVIDER` + `IMSG_TRANSCRIBE_API_KEY` are set, `transcribeAudioBest` falls back to an opt-in OpenAI-compatible cloud endpoint (`getTranscribeCloudConfig` in `src/config.ts`). `get_attachment` surfaces `transcriptSource: "local"|"cloud"`; audio leaves the device only when the cloud path fires.
